...
func UserSeacrhData(w http.ResponseWriter, r *http.Request) {
	...
	_con := model1.ModelUser_init{DB: db}
	result, err, totalDataSearch := _con.SearchUserModels(Keyword.Keyword, Keyword.Page, Keyword.Show_data)
	if err != nil {
		log.Println(err.Error())
	}
	...
	if r.Method == "POST" {
		if result == nil {
			...
			_response.Status = http.StatusBadRequest
			_response.Message = "User Data doesn't exists."
			response.ResponseJson(w, _response.Status, _response)
		} else {
			_response.Status = http.StatusOK
			_response.Message = "Success"
