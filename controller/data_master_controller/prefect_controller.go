...
func SearchDataPrefecture(w http.ResponseWriter, r *http.Request) {
	...
	_con := model1.ModelPref_init{DB: db}
	result, err, totalDataSearch := _con.SearchPrefectureModels(Keyword.Keyword, Keyword.Page, Keyword.Show_data)
	if err != nil {
		log.Println(err.Error())
	}
	...
	if r.Method == "POST" {
		if result == nil {
			...
			_response.Status = http.StatusBadRequest
			_response.Message = "Prefecture Data doesn't exists."
			response.ResponseJson(w, _response.Status, _response)
		} else {
			_response.Status = http.StatusOK
			_response.Message = "Success"
